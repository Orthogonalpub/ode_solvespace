...
static ShowHideButton shadedButton = {&(SS.GW.showShaded), "shaded", "shaded view of solid model"};
...
void TextWindow::Init() {
    if(!window) {
        ...
        if(window) {
            ...
            window->SetMinContentSize(450, 370);
        }
    }
...
std::shared_ptr<Pixmap> TextWindow::HsvPattern2d(int w, int h) {
    ...
    for(size_t j = 0; j < pixmap->height; j++) {
        ...
        for(size_t i = 0; i < pixmap->width; i++) {
            Vector hsv =
                Vector::From(6.0 * i / (pixmap->width - 1), 1.0 * j / (pixmap->height - 1), 1);
...
bool TextWindow::DrawOrHitTestColorPicker(UiCanvas *uiCanvas, DrawOrHitHow how, bool leftDown,
    ...
    if(how == PAINT) {
        ...
        if(x >= hx && x <= hxm && y >= hy && y <= hym) {
            ...
            Vector rgb =
                HsvToRgb(Vector::From(6 * editControl.colorPicker.h, editControl.colorPicker.s,
            ...
            for(c = 0; c < min(((int)width / CHAR_WIDTH_) + 1, (int)MAX_COLS); c++) {
                ...
                if(a == 0) {
                    ...
                    if(bg == 'z') {
                        ...
                        bgRgb =
                            RgbaColor::FromFloat(bgColorTable[bg * 3 + 0], bgColorTable[bg * 3 + 1],
                                                 bgColorTable[bg * 3 + 2]);
                    }
                    ...
                    RgbaColor fgRgb =
                        RgbaColor::FromFloat(fgColorTable[fg * 3 + 0], fgColorTable[fg * 3 + 1],
                                             fgColorTable[fg * 3 + 2]);
                    if(text[r][c] != ' ') {
                    ...
                    if(meta[r][c].link && meta[r][c].link != 'n' &&
                       (r == hoveredRow && c == hoveredCol)) {
                        ...
                        uiCanvas.DrawLine(LEFT_MARGIN + cs * CHAR_WIDTH_, yp,
                                          LEFT_MARGIN + cf * CHAR_WIDTH_, yp, fgRgb);
                    }
                }
            }
        }

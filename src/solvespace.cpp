...
void SolveSpaceUI::Init() {
    ...
    backgroundColor = settings->ThawColor("BackgroundColor", RGBi(238, 242, 245));
...
std::string SolveSpaceUI::MmToStringSI(double v, int dim) {
    ...
    if(fabs(v) > 0.0) {
        int sdeg = 0;
        std::tie(sdeg, unit) =
            inches ? SelectSIPrefixInch(vdeg / dim) : SelectSIPrefixMm(vdeg, dim);
        v /= pow(10.0, sdeg * dim);
    }
...
bool SolveSpaceUI::OkayToStartNewFile() {
    ...
    case MessageDialog::Response::YES: return GetFilenameAndSave(/*saveAs=*/false);

    case MessageDialog::Response::NO: RemoveAutosave(); return true;
...
void SolveSpaceUI::MenuFile(Command id) {
    ...
    case Command::SAVE: SS.GetFilenameAndSave(/*saveAs=*/false); break;

    case Command::SAVE_AS: SS.GetFilenameAndSave(/*saveAs=*/true); break;
...
void SolveSpaceUI::MenuAnalyze(Command id) {
    ...
    case Command::INTERFERENCE: {
        ...
        bool inters, leaks;
        root->MakeCertainEdgesInto(&(SS.nakedEdges), EdgeKind::SELF_INTER,
                                   /*coplanarIsInter=*/false, &inters, &leaks);

        SS.GW.Invalidate();

        if(inters) {
            Error("%d edges interfere with other triangles, bad.", SS.nakedEdges.l.n);
        } else {
...
void SolveSpaceUI::ShowNakedEdges(bool reportOnlyWhenNotOkay) {
    ...
    if(SS.nakedEdges.l.IsEmpty()) {
        Message(_("%s\n\n%s\n\nZero problematic edges, good.%s"), intersMsg, leaksMsg,
                cntMsg.c_str());
    } else {
